...
export function viewFunction(contractId, methodName, args) {
...
export async function getTokenBalance(id) {
 if (!wallet.isSignedIn()) return '0';
  ...
  if (id == CONFIG.WRAP_NEAR_CONTRACT_ID) {
    ...
    result = await viewFunction(id, "ft_balance_of", {
...
export async function ftGetTokenMetadata(id) {
  const result = await viewFunction(id, "ft_metadata");
  return result;
}
...
export function UseTokensData(tokenIds) {
  ...
  async function getTokensData() {
    const promisTokens = tokenIds.map(async (id) => {
      ...
      if (id == CONFIG.WRAP_NEAR_CONTRACT_ID) {
        ...
        meta = await ftGetTokenMetadata(id);
      }
      return {
        ...meta,
        balance,
    ...
    const result = await Promise.all(promisTokens);
    setTokenList(result);
  }
  return tokenList;
}
export const wnearMetadata = {
  id: CONFIG.WRAP_NEAR_CONTRACT_ID,
  name: "wNEAR",
  symbol: "wNEAR",
  decimals: 24,
...
async function createNewTransaction({receiverId, nonceOffset, actions}) {
  ...
  let accessKey = await walletAccount.accessKeyForTransaction(
    receiverId,
    actions,
    localKey
  );
